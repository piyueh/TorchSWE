...
cdef class InflowCuPy:
    ...
    def __init__(
        ...
        if ornt == 0:  # west
            _inflow_bc_set_west(self, Q, Bx, Qmx, Qpx, H, Hmx, Hpx, ngh, comp)
        elif ornt == 1:  # east
            _inflow_bc_set_east(self, Q, Bx, Qmx, Qpx, H, Hmx, Hpx, ngh, comp)
        elif ornt == 2:  # south
            _inflow_bc_set_south(self, Q, By, Qmy, Qpy, H, Hmy, Hpy, ngh, comp)
        elif ornt == 3:  # north
            _inflow_bc_set_north(self, Q, By, Qmy, Qpy, H, Hmy, Hpy, ngh, comp)
        else:
...
cdef class InflowCuPyWH(InflowCuPy):
    """Inflow (const. non-conservative quantities) boundary condition for updating w and h.
    """
...
cdef class InflowCuPyOther(InflowCuPy):
    """Inflow (const. non-conservative quantities) boundary condition for updating hu or hv.
    """
...
cdef _inflow_bc_w_h_kernel = cupy.ElementwiseKernel(
    ...
    """
        ...
        hbco = val;  // `val` (i.e., h bc) must be non-negative; otherwise, it's users' fault
        wbco = val + bbc;
...
def inflow_factory(ornt, comp, states, topo, theta, tol, drytol, const double val, *args, **kwargs):
