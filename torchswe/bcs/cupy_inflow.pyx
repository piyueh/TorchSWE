...
cdef class InflowBC:
    """Inflow (conservative non-quantities) boundary condition for the CuPy backend.
    """

    # read-only auxiliary data
    cdef object w  # water elevation
    cdef object b  # topography elevation
    ...
    def __call__(self):
        self.kernel(self.val, self.w, self.b, self.qbcm1, self.qbcm2)


cdef _inflow_bc_w_kernel = cupy.ElementwiseKernel(
    "T val, T w, T b",  # w is not used; keeping it just for a consistent signature
    "T qbcm1, T qbcm2",
    """
        qbcm1 = b + val;
        qbcm2 = qbcm1;
...
cdef void _inflow_bc_set_west(
    ...
    bc.val      = cupy.full_like(bc.w, val, dtype=Q.dtype)
    bc.kernel   = _inflow_bc_w_kernel if comp == 0 else _inflow_bc_kernel
...
cdef inline void _inflow_bc_factory(
    ...
    if ornt == 0:  # west
        _inflow_bc_set_west(bc, Q, B, Bx, val, ngh, comp)
    elif ornt == 1:  # east
        _inflow_bc_set_east(bc, Q, B, Bx, val, ngh, comp)
    elif ornt == 2:  # south
        _inflow_bc_set_south(bc, Q, B, By, val, ngh, comp)
    elif ornt == 3:  # north
        _inflow_bc_set_north(bc, Q, B, By, val, ngh, comp)
    else:
...
def inflow_bc_factory(ornt, comp, states, topo, val, *args, **kwargs):
    ...
    bc = InflowBC()
    _inflow_bc_factory(bc, Q, B, Bx, By, val, ngh, comp, ornt)
