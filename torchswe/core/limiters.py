...
def minmod_slope(states: _States, theta: float, tol: float = 1e-12) -> _States:
    ...
    states.slp.x.w = minmod_slope_x_one_comp(states.q.w, dx, states.ngh, theta, tol)
    states.slp.x.hu = minmod_slope_x_one_comp(states.q.hu, dx, states.ngh, theta, tol)
    states.slp.x.hv = minmod_slope_x_one_comp(states.q.hv, dx, states.ngh, theta, tol)
    states.slp.y.w = minmod_slope_y_one_comp(states.q.w, dy, states.ngh, theta, tol)
    states.slp.y.hu = minmod_slope_y_one_comp(states.q.hu, dy, states.ngh, theta, tol)
    states.slp.y.hv = minmod_slope_y_one_comp(states.q.hv, dy, states.ngh, theta, tol)

    return states


def minmod_slope_x_one_comp(q: _nplike.ndarray, dx: float, ngh: int, theta: float, tol: float):
    ...
    slpx = _nplike.maximum(
        _nplike.minimum(
            _nplike.minimum(theta*slpx, (1.+slpx)/2.),
            _nplike.array(theta)),
        _nplike.array(0.)
    )
...
def minmod_slope_y_one_comp(q: _nplike.ndarray, dy: float, ngh: int, theta: float, tol: float):
    """Minmod slope in x direction for only one conservative quantity.
    ...
    slpy = _nplike.maximum(
        _nplike.minimum(
            _nplike.minimum(theta*slpy, (1.+slpy)/2.),
            _nplike.array(theta)),
        _nplike.array(0.)
    )
